...
const en = {
  ...
  "write.kindSms": "SMS",
  "write.kindMailto": "Email message",
  "write.kindGeo": "Location",
  "write.kindVcard": "vCard",
  "write.kindWifi": "WiFi credentials",
  "write.kindSocial": "Social media",
  "write.socialHandlePlaceholder": "Username / ID / phone number — see preview below",
  "write.socialPreviewLabel": "Will be written as",
  "write.socialPlainTextNote":
    "This platform has no way to build a working link from just this — it'll be written as plain text instead: readable when scanned, but not tappable.",
  "write.kindRaw": "Custom (raw record)",
  "write.rawHint":
    "Advanced: builds an NDEF record exactly as specified, with no validation that the TNF/type/payload combination is meaningful. Requires understanding the NDEF record format.",
  "write.rawTnf0": "Empty (0x00)",
  "write.rawTnf1": "Well-Known (0x01)",
  "write.rawTnf2": "MIME Media (0x02)",
  "write.rawTnf3": "Absolute URI (0x03)",
  "write.rawTnf4": "External (0x04)",
  "write.rawTnf5": "Unknown (0x05)",
  "write.rawTnf6": "Unchanged (0x06)",
  "write.rawTypePlaceholder": "Record type, e.g. \"T\", \"U\", \"application/json\"",
  "write.rawPayloadTextPlaceholder": "Payload text (UTF-8 encoded)",
  "write.rawPayloadHexPlaceholder": "Payload as hex bytes, e.g. 48656C6C6F",
  "write.rawPayloadHexInvalid": "Must be an even number of hex digits (0-9, A-F)",
  "write.latPlaceholder": "Latitude, e.g. 40.748817",
  "write.lngPlaceholder": "Longitude, e.g. -73.985428",
  "write.geoUseCurrentLocation": "Use current location",
  ...
  "write.vcardSwitchToRaw":
    "Edit as raw text (for address, birthday, nickname, and other fields not in the form, multiple phone numbers, TYPE= parameters, etc.)",
  "write.vcardSwitchToForm": "Switch back to form",
  "write.vcardRawPlaceholder":
    "BEGIN:VCARD\nVERSION:3.0\nFN:Jane Doe\nTEL;TYPE=CELL:+1-212-555-0100\nTEL;TYPE=WORK,VOICE:+1-212-555-0101\nEND:VCARD",
  "write.wifiSsid": "SSID",
  "write.wifiPassword": "Password",
  "write.wifiSecurityWpa2": "WPA2-Personal (recommended)",
  "write.wifiSecurityWpa": "WPA-Personal",
  "write.wifiSecurityWep": "WEP",
  "write.wifiSecurityOpen": "Open (no password)",
  "write.urlPlaceholder": "https://example.com",
  "write.telPlaceholder": "+12125550100",
  "write.smsToPlaceholder": "Recipient, e.g. +12125550100",
  "write.smsBodyPlaceholder": "Message text (optional)",
  "write.mailtoToPlaceholder": "Recipient email, e.g. someone@example.com",
  "write.mailtoSubjectPlaceholder": "Subject (optional)",
  "write.mailtoBodyPlaceholder": "Message body (optional)",
  ...
  "write.advancedSettings": "Advanced settings",
...
const zh: Record<TranslationKey, Entry> = {
  ...
  "write.kindSocial": "社交媒体",
  "write.socialHandlePlaceholder": "用户名 / ID / 手机号，具体看下方预览",
  "write.socialPreviewLabel": "实际写入",
  "write.socialPlainTextNote": "这个平台没法只靠这个内容拼出能点开的链接，会作为纯文本写入——扫到能读到内容，但点不开。",
  "write.kindRaw": "自定义（原始记录）",
  "write.rawHint": "高级功能：会完全按你指定的内容构建 NDEF 记录，不会检查 TNF / 类型 / 负载的组合是否有意义，需要你了解 NDEF 记录格式。",
  "write.rawTnf0": "空记录 (0x00)",
  "write.rawTnf1": "已知类型 (0x01)",
  "write.rawTnf2": "MIME 类型 (0x02)",
  "write.rawTnf3": "绝对 URI (0x03)",
  "write.rawTnf4": "外部类型 (0x04)",
  "write.rawTnf5": "未知类型 (0x05)",
  "write.rawTnf6": "延续记录 (0x06)",
  "write.rawTypePlaceholder": "记录类型，如 \"T\"、\"U\"、\"application/json\"",
  "write.rawPayloadTextPlaceholder": "负载内容（按 UTF-8 编码）",
  "write.rawPayloadHexPlaceholder": "负载的十六进制字节，如 48656C6C6F",
  "write.rawPayloadHexInvalid": "必须是偶数个十六进制字符（0-9、A-F）",
  "write.latPlaceholder": "纬度，如 40.748817",
  "write.lngPlaceholder": "经度，如 -73.985428",
  "write.geoUseCurrentLocation": "获取当前位置",
  ...
  "write.vcardUrl": "网站",
  "write.vcardNote": "备注",
  "write.vcardSwitchToRaw": "编辑原始文本（地址、生日、昵称等表单里没有的字段，以及多个电话号码、TYPE= 参数等高级用法）",
  "write.vcardSwitchToForm": "切换回表单编辑",
  "write.vcardRawPlaceholder":
    "BEGIN:VCARD\nVERSION:3.0\nFN:张三\nTEL;TYPE=CELL:+1-212-555-0100\nTEL;TYPE=WORK,VOICE:+1-212-555-0101\nEND:VCARD",
  "write.wifiSsid": "SSID",
  "write.wifiPassword": "密码",
  "write.wifiSecurityWpa2": "WPA2-Personal（推荐）",
  "write.wifiSecurityWpa": "WPA-Personal",
  "write.wifiSecurityWep": "WEP",
  "write.wifiSecurityOpen": "开放网络（无密码）",
  "write.urlPlaceholder": "https://example.com",
  "write.telPlaceholder": "+12125550100",
  "write.smsToPlaceholder": "发送至，例如 +12125550100",
  "write.smsBodyPlaceholder": "短信内容（可选）",
  "write.mailtoToPlaceholder": "收件人邮箱，例如 someone@example.com",
  "write.mailtoSubjectPlaceholder": "邮件标题（可选）",
  "write.mailtoBodyPlaceholder": "邮件内容（可选）",
